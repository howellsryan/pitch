...
| Action | Command |
|--------|---------|
| Build | `npm run build` (or `python3 src/build.py`), from the repo root |
| Validate | `npm run validate` (or `node src/validate.js`) — 1180 checks, **0 failures** |
| Output | `index.html` at the repo root (~878KB) — gitignored, regenerated by every build |
| Intermediate | `.build/bundle_final.js` — gitignored scratch file |
| Deploy | `npm run deploy` (builds, then `wrangler deploy`) — needs `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` |
...
## Project
Single-file football career manager. Vanilla JS + IndexedDB + CSS. No framework
yet — see the rebuild plan for the move to Vite + Svelte. `src/build.py`
concatenates 22 source files in strict order, strips ES modules, emits one
HTML file to the repo root.
