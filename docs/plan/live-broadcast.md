...
`isBroadcastReady` gates the next result tick and the final full-time transition. Watch uses a single speed. Pause freezes both clocks. Substitutions and formation changes remain authoritative immediately but presentation lineup replacement waits until the already-resolved scene finishes, preserving named scorers and in-flight receivers. Automatic lineup changes are passed through the same adapter.

### Current text commentary and matchday controls

`MatchCommentary.svelte` renders the human-paced story from `liveMatchStory.js` through `broadcastFrameSemantics.js`. MatchScreen advances that adapter explicitly for every animation step and when admitting a ledger record; rendering a Svelte derived value must not drive its state transitions.

Each goal produces a **single reveal with team, scorer and minute**. MatchScreen checks that identity against the queued authoritative event before updating the displayed score. The next engine phase waits until the goal passage and its 4.2-second notice have been presented. The final result waits too. A lingering GOAL headline must never unlock a later goal. The goals-so-far list contains only revealed events and allows repeated scorers in the same displayed minute.

Your team's goals use a green GOAL takeover, restrained confetti, a score update and optional haptics. Opponent goals use a subdued red Goal conceded panel without confetti or celebration haptics. Both include scorer, team and the home–away score. Reduced motion keeps the complete message visible and removes animation. Pausing freezes the notice duration and its animation.

Half-time is an indefinite hold. A goal at the phase-60 boundary finishes its commentary and notice first, then MatchScreen pauses both clocks. **Start second half** explicitly calls `resumeBroadcastHalfTime`, swaps ends once and gives the other team the kickoff. Opening and closing Tactics during the break must keep the match paused. Restored second-half sessions initialise the presentation with `secondHalfStarted` so they do not trigger a second break.

The score stays visible while scrolling the mobile match panel; Home/Away and You labels identify sides independently of colour. Match controls have 48px targets and stay above the safe area. The shared mobile navigation has larger, higher-contrast labels; the empty transfer-deals view links directly to player search.
...
After the manager explicitly chooses **Start second half**:
