...
import { derivedRestart, makeBroadcastFrame } from './matchPresentation.js';
...
describe('makeBroadcastFrame', () => {
  ...
  it('derives a paced, deterministic restart without changing the result', () => { expect(derivedRestart(12)).toBeNull(); expect(derivedRestart(13)).toEqual({ type:'free-kick', variant:1 }); const frame = makeBroadcastFrame({ ...base, phase:13, restart:derivedRestart(13) }); expect(frame.action).toContain('FREE KICK'); });
});
